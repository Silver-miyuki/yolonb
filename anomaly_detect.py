...
def detect_single_video(video_path, output_path):
    ...
    _pre_gray, pre_kpts, center_his = None, [], []
    ...
    while True:
        ...
        # 初始化缓存
        while len(center_his) < len(person):
            center_his.append(deque(maxlen=5))
        while len(center_his) > len(person):
            center_his.pop()
        while len(pre_kpts) < len(person):
            pre_kpts.append(None)
        while len(pre_kpts) > len(person):
            pre_kpts.pop()
        ...
        if fall or run or fight:
            out.write(frame)
        _pre_gray, pre_kpts = cur_gray.copy(), cur_kpt
